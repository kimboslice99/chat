var WebRTC = {
        ...
        window.addEventListener("chat-active", () => {
            ...
            socket.once('signaling-available', (data) => {
                ...
                if (data.iceServers && data.iceServers.length > 0) {
                    this.config = this.mergeIceServers(this.config, data);
        ...
        socket.on('signal', async (data) => {
            ...
            if (signal.sdp) {
                ...
                if (signal.sdp.type === 'offer') {
                    ...
                    socket.emit('signal', { target: from, signal: { sdp: this.peerConnections[from].localDescription } });
                }
        ...
        socket.on('user-ready', async (id) => {
            ...
            try {
                ...
                Chat.socket.emit('signal', {
                    target: id, signal: { sdp: this.peerConnections[id].localDescription }
                });
            } catch (error) {
                console.error(error);
            }
     ...
     */
    createPeerConnection: async function(id) {
        this.peerConnections[id] = new RTCPeerConnection(this.config);

        // Add local stream to the connection
        if (this.localStream) {
            this.localStream.getTracks().forEach(track => this.peerConnections[id].addTrack(track, this.localStream));
        }
    ...
    populateOptions: async function(){
